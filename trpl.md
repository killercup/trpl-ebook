...
[readme--rust]: http://rust-lang.org
...
* [Learn Rust][readme--lr] - Learn Rust programming through small projects.
...
This program makes a [variable binding][readme--var] named `x`. The value of this
binding is a `Vec<T>`, a ‘vector’, that we create through a [macro][readme--macro]
...
ignore it for now, or check out [‘The Stack and the Heap’][readme--heap]. As a systems
...
[readme--var]: #sec--variable-bindings
[readme--macro]: #sec--macros
[readme--heap]: #sec--the-stack-and-the-heap
...
interact with the ownership system by [‘borrowing’][readme--borrowing] what they point
...
[readme--borrowing]: #sec--references-and-borrowing
...
Rust has [move semantics][readme--move] by default, so if we want to make a copy of some
data, we call the `clone()` method. In this example, `y` is no longer a reference
to the vector stored in `x`, but a copy of its first element, `"Hello"`. Now
that we don’t have a reference, our `push()` works just fine.

[readme--move]: #move-semantics
...
If you're concerned about the [potential insecurity][installing-rust.md--insecurity] of using `curl
...
[installing-rust.md--insecurity]: http://curlpipesh.tumblr.com

If you're on Windows, please download either the [32-bit installer][installing-rust.md--win32] or
the [64-bit installer][installing-rust.md--win64] and run it.

[installing-rust.md--win32]: https://static.rust-lang.org/dist/rust-1.0.0-i686-pc-windows-gnu.msi
[installing-rust.md--win64]: https://static.rust-lang.org/dist/rust-1.0.0-x86_64-pc-windows-gnu.msi
...
documentation on [building Rust from Source][installing-rust.md--from-source], or [the official
binary downloads][installing-rust.md--install-page].

[installing-rust.md--from-source]: https://github.com/rust-lang/rust#building-from-source
[installing-rust.md--install-page]: http://www.rust-lang.org/install.html
...
[the #rust IRC channel on irc.mozilla.org][installing-rust.md--irc], which you can access through
[Mibbit][installing-rust.md--mibbit]. Click that link, and you'll be chatting with other Rustaceans
(a silly nickname we call ourselves), and we can help you out. Other great
resources include [the user’s forum][installing-rust.md--users], and
[Stack Overflow][installing-rust.md--stackoverflow].

[installing-rust.md--irc]: irc://irc.mozilla.org/#rust
[installing-rust.md--mibbit]: http://chat.mibbit.com/?server=irc.mozilla.org&channel=%23rust
[installing-rust.md--users]: http://users.rust-lang.org/ 
[installing-rust.md--stackoverflow]: http://stackoverflow.com/questions/tagged/rust
...
[SolidOak][hello-world.md--solidoak], or wherever plugins are for your favorite IDE. There are
a number of extensions of varying quality in development by the community. The
Rust team also ships [plugins for various editors][hello-world.md--plugins]. Configuring your
editor or IDE is out of the scope of this tutorial, so check the documentation
for your setup, specifically.

[hello-world.md--solidoak]: https://github.com/oakes/SolidOak
[hello-world.md--plugins]: https://github.com/rust-lang/rust/blob/master/src/etc/CONFIGS.md
...
editors][hello-world.md--configs].

[hello-world.md--configs]: https://github.com/rust-lang/rust/tree/master/src/etc/CONFIGS.md

The second point is the `println!()` part. This is calling a Rust [macro][hello-world.md--macro],
...
[Cargo][hello-cargo.md--cratesio] is a tool that Rustaceans use to help manage their Rust
...
[hello-cargo.md--cratesio]: http://doc.crates.io
...
README][hello-cargo.md--cargoreadme] for specific instructions about installing it.

[hello-cargo.md--cargoreadme]: https://github.com/rust-lang/cargo#installing-cargo-from-nightlies
...
with a [`[[lib]]` or `[[bin]]`][hello-cargo.md--crates-custom] key in the TOML file described below.

[hello-cargo.md--crates-custom]: http://doc.crates.io/manifest.html#configuring-a-target
...
This file is in the [TOML][hello-cargo.md--toml] format. Let’s let it explain itself to you:
...
[hello-cargo.md--toml]: https://github.com/toml-lang/toml
...
has its own [guide][hello-cargo.md--guide] which covers Cargo’s features in much more depth.

[hello-cargo.md--guide]: http://doc.crates.io/guide.html

Now that you’ve got the tools down, let’s actually learn more about the Rust
language itself. These are the basics that will serve you well through the rest
of your time with Rust.

You have two options: Dive into a project with ‘[Learn Rust][hello-cargo.md--learnrust]’, or
start from the bottom and work your way up with ‘[Syntax and
Semantics][hello-cargo.md--syntax]’. More experienced systems programmers will probably prefer
‘Learn Rust’, while those from dynamic backgrounds may enjoy either. Different
people learn differently! Choose whatever’s right for you.

[hello-cargo.md--learnrust]: #sec--learn-rust
[hello-cargo.md--syntax]: #sec--syntax-and-semantics
...
out [Syntax and Semantics][learn-rust.md--ss].

[learn-rust.md--ss]: #sec--syntax-and-semantics
...
into every program, [the ‘prelude’][guessing-game.md--prelude]. If it’s not in the prelude,
...
a value: in this case, an [`io::Result`][guessing-game.md--ioresult]. Rust has a number of
types named `Result` in its standard library: a generic [`Result`][guessing-game.md--result],
...
[`ok()` method][guessing-game.md--ok] returns a value which has another method defined on it:
`expect()`. The [`expect()` method][guessing-game.md--expect] takes a value it’s called on, and
if it isn’t a successful one, [`panic!`][guessing-game.md--panic]s with a message you
passed it. A `panic!` like this will cause our program to crash, displaying
the message.

[guessing-game.md--ok]: http://doc.rust-lang.org/std/result/enum.Result.html#method.ok
...
of versions. [Cargo’s documentation][guessing-game.md--cargodoc] contains more details.
...
strings][guessing-game.md--parse] parses a string into some kind of number. Since it can parse a
...
integer. Rust has [a number of built-in number types][guessing-game.md--number], but we’ve
...
1965, but we’ll use a lightly adapted version from [this paper][dining-philosophers.md--paper] by Tony
Hoare in 1985.

[dining-philosophers.md--paper]: http://www.usingcsp.com/cspbook.pdf
...
Here, we make a [`struct`][dining-philosophers.md--struct] to represent a philosopher. For now,
a name is all we need. We choose the [`String`][dining-philosophers.md--string] type for the name,
...
[dining-philosophers.md--struct]: #sec--structs
[dining-philosophers.md--string]: #sec--strings
...
[`for`][dining-philosophers.md--for] loop to iterate through the vector, getting a reference to each
philosopher in turn.

[dining-philosophers.md--for]: #sec--for-loops
...
case, the error that could happen is that the mutex is [‘poisoned’][dining-philosophers.md--poison],
which is what happens when the thread panics while the lock is held. Since this
shouldn’t happen, we just use `unwrap()`.

[dining-philosophers.md--poison]: http://doc.rust-lang.org/std/sync/struct.Mutex.html#poisoning
...
There is a whole [chapter devoted to FFI][rust-inside-other-languages.md--ffi] and its specifics elsewhere in
the book, but in this chapter, we’ll examine this particular use-case of FFI,
with examples in Ruby, Python, and JavaScript.

[rust-inside-other-languages.md--ffi]: #sec--ffi
...
there's a lot more that we could do here. Check out the [FFI][rust-inside-other-languages.md--ffi]
...
number comes from 2^30^, the number of bytes in a gigabyte.
...
We have 2^30^ in our hypothetical computer with 1GB of RAM. And since
...
allocated on the heap, so the value of `x` is 2^30^, the memory
...
them. There’s a gap between 2^30^ and (2^30^) - 3 which isn’t
...
from 2^30^. It’s easier to just write that than `1,073,741,823`. In any
...
[the-stack-and-the-heap.md--wilson]: http://www.cs.northwestern.edu/~pdinda/icsclass/doc/dsa.pdf
...
[conditional-compilation.md--features]: http://doc.crates.io/manifest.html#the-%5Bfeatures%5D-section
...
[documentation.md--rc-new]: http://doc.rust-lang.org/nightly/std/rc/struct.Rc.html#method.new
...
[`Send`](http://doc.rust-lang.org/std/marker/trait.Send.html). When a type `T` implements `Send`, it indicates
...
[`lock`](http://doc.rust-lang.org/std/sync/struct.Mutex.html#method.lock)
...
The [`Debug`](http://doc.rust-lang.org/std/fmt/trait.Debug.html) trait is what lets us print the enum value using the `{:?}` format operation.
...
`try!` makes use of [`From<Error>`](http://doc.rust-lang.org/std/convert/trait.From.html) to determine
...
expression is a ‘[pattern][variable-bindings.md--pattern]’, not just a variable name. This means we
can do things like:
...
like [`&str`][primitive-types.md--strings]. As such, we’ll just leave it at that.
...
The expression is an [iterator][for-loops.md--iterator]. The iterator gives back a series of
...
[for-loops.md--iterator]: #sec--iterators
...
Both `continue` and `break` are valid in both `while` loops and [`for` loops][while-loops.md--for].

[while-loops.md--for]: #sec--for-loops
...
When `v` comes into scope, a new [`Vec<T>`][ownership.md--vect] is created. In this case, the
vector also allocates space on [the heap][ownership.md--heap], for the three elements. When
...
[ownership.md--vect]: http://doc.rust-lang.org/std/vec/struct.Vec.html
...
You’ll also need explicit lifetimes when working with [`struct`][lifetimes.md--structs]s:
...
[mutability.md--borrowing]: #The-Rules
...
[section on traits][generics.md--traits].
...
the [traits chapter][closures.md--traits] before this one, as well as the chapter on [trait
...
[macros.md--hygienic macro system]: http://en.wikipedia.org/wiki/Hygienic_macro
...
[nightly-rust.md--install-page]: http://www.rust-lang.org/install.html
...
[nightly-rust.md--users]: http://users.rust-lang.org/ 
...
[`rustc::plugin`](http://doc.rust-lang.org/rustc/plugin/index.html) documentation for more about the
...
[libstd implementation](http://doc.rust-lang.org/std/rt/unwind/index.html) for more
...
not sufficient, then [libcore](http://doc.rust-lang.org/core/index.html) is designed to be used
instead.
