...
\KeywordTok{fn} \NormalTok{call_with_ref<}\OtherTok{'a}\NormalTok{, F>(some_closure:F) -> }\DataTypeTok{i32} 
    \KeywordTok{where} \NormalTok{F: }\BuiltInTok{Fn}\NormalTok{(&}\OtherTok{'a} \DecValTok{32}\NormalTok{) -> }\DataTypeTok{i32} \NormalTok{\{}
\end{Highlighting}
\end{Shaded}

However this presents a problem with in our case. When you specify the
explict lifetime on a function it binds that lifetime to the
...
compiles and excutes as we expect.
