...
We’ll need to take user input, and then print the result as output. As such, we need the `io` library from the standard library. Rust only imports a few things into every program, [the ‘prelude’](../std/prelude/index.html). If it’s not in the prelude, you’ll have to `use` it directly.
...
`String` is a string type, provided by the standard library. A [`String`](../std/string/struct.String.html) is a growable, UTF-8 encoded bit of text.
...
This particular function returns a handle to the standard input for your terminal. More specifically, a [std::io::Stdin](../std/io/struct.Stdin.html).
...
Here, we call the [`read_line()`](../std/io/struct.Stdin.html#method.read_line) method on our handle. [Method](methods.html)s are like associated functions, but are only available on a particular instance of a type, rather than the type itself. We’re also passing one argument to `read_line()`: `&mut guess`.
...
But that gets hard to read. So we’ve split it up, three lines for three method calls. We already talked about `read_line()`, but what about `ok()` and `expect()`? Well, we already mentioned that `read_line()` puts what the user types into the `&mut String` we pass it. But it also returns a value: in this case, an [`io::Result`](../std/io/type.Result.html). Rust has a number of types named `Result` in its standard library: a generic [`Result`](../std/result/enum.Result.html), and then specific versions for sub-libraries, like `io::Result`.

The purpose of these `Result` types is to encode error handling information. Values of the `Result` type, like any type, have methods defined on them. In this case, `io::Result` has an `ok()` method, which says ‘we want to assume this value is a successful one. If not, just throw away the error information’. Why throw it away? Well, for a basic program, we just want to print a generic error, as basically any issue means we can’t continue. The [`ok()` method](../std/result/enum.Result.html#method.ok) returns a value which has another method defined on it: `expect()`. The [`expect()` method](../std/option/enum.Option.html#method.expect) takes a value it’s called on, and if it isn’t a successful one, [`panic!`](error-handling.html)s with a message you passed it. A `panic!` like this will cause our program to crash, displaying the message.
...
The [`Ordering`](../std/cmp/enum.Ordering.html) enum has three possible variants: `Less`, `Equal`, and `Greater`. The `match` statement takes a value of a type, and lets you create an ‘arm’ for each possible value. Since we have three types of `Ordering`, we have three arms:
...
The call to `lock()` might fail, and if it does, we want to crash. In this case, the error that could happen is that the mutex is [‘poisoned’](../std/sync/struct.Mutex.html#poisoning), which is what happens when the thread panics while the lock is held. Since this shouldn’t happen, we just use `unwrap()`.
...
In Rust, you can allocate memory on the heap with the [`Box<T>` type](../std/boxed/index.html). Here’s an example:
...
The [`Borrow`](../std/borrow/trait.Borrow.html) and [`AsRef`](../std/convert/trait.AsRef.html) traits are very similar, but different. Here’s a quick refresher on what these two traits mean.
...
For example, [`HashMap`](../std/collections/struct.HashMap.html) has a [`get` method](../std/collections/struct.HashMap.html#method.get) which uses `Borrow`:
...
You can find more documentation for `bool`s [in the standard library documentation](../std/primitive.bool.html).
...
You can find more documentation for `char`s [in the standard library documentation](../std/primitive.char.html).
...
You can find more documentation for `array`s [in the standard library documentation](../std/primitive.array.html).
...
You can find more documentation for slices [in the standard library documentation](../std/primitive.slice.html).
...
You can find more documentation for `str` [in the standard library documentation](../std/primitive.str.html).
...
You can find more documentation for tuples [in the standard library documentation](../std/primitive.tuple.html).
...
When `v` comes into scope, a new [`Vec<T>`](../std/vec/struct.Vec.html) is created. In this case, the vector also allocates space on [the heap](the-stack-and-the-heap.html), for the three elements. When `v` goes out of scope at the end of `foo()`, Rust will clean up everything related to the vector, even the heap-allocated memory. This happens deterministically, at the end of the scope.
...
However, when we say something is ‘immutable’ in Rust, that doesn’t mean that it’s not able to be changed: We mean something has ‘exterior mutability’. Consider, for example, [`Arc<T>`](../std/sync/struct.Arc.html):
...
Other types, like the ones in the [`std::cell`](../std/cell/index.html) module, have the opposite: interior mutability. For example:
...
A ‘vector’ is a dynamic or ‘growable’ array, implemented as the standard library type [`Vec<T>`](../std/vec/index.html). The `T` means that we can have vectors of any type (see the chapter on [generics](generics.html) for more). Vectors always allocate their data on the heap. You can create them with the `vec!` macro:
...
Vectors have many more useful methods, which you can read about in [their API documentation](../std/vec/index.html).
...
Now that we’ve discussed traits, let’s talk about a particular trait provided by the Rust standard library, [`Drop`](../std/ops/trait.Drop.html). The `Drop` trait provides a way to run some code when a value goes out of scope. For example:
...
So what is `Drop` good for? Generally, `Drop` is used to clean up any resources associated with a `struct`. For example, the [`Arc<T>` type](../std/sync/struct.Arc.html) is a reference-counted type. When `Drop` is called, it will decrement the reference count, and if the total number of references is zero, will clean up the underlying value.
...
Let’s start simple, with the runtime representation of a trait object. The `std::raw` module contains structs with layouts that are the same as the complicated built-in types, [including trait objects](../std/raw/struct.TraitObject.html):
...
Rust attributes are used for a number of different things. There is a full list of attributes [in the reference](../reference.html#attributes). Currently, you are not allowed to create your own attributes, the Rust compiler defines them.
...
This creates a specialized version of the `Result` type, which always has a `ConcreteError` for the `E` part of `Result<T, E>`. This is commonly used in the standard library to create custom errors for each subsection. For example, [io::Result](../std/io/type.Result.html).
...
There are a number of operators that can be overloaded this way, and all of their associated traits live in the [`std::ops`](../std/ops/index.html) module. Check out its documentation for the full list.

Implementing these traits follows a pattern. Let’s look at [`Add`](../std/ops/trait.Add.html) in more detail:
...
The standard library provides a special trait, [`Deref`](../std/ops/trait.Deref.html). It’s normally used to overload `*`, the dereference operator:
...
This is like a `match` expression arm, but the matching happens on Rust syntax trees, at compile time. The semicolon is optional on the last (here, only) case. The "pattern" on the left-hand side of `=>` is known as a ‘matcher’. These have [their own little grammar](../reference.html#macros) within the language.
...
This holds for `let` bindings and loop labels, but not for [items](../reference.html#items). So the following code does compile:
...
-   `item`: an [item](../reference.html#items). Examples: `fn foo() { }`; `struct Bar;`.
...
The Rust Reference has a [listing of macro-related attributes](../reference.html#macro-related-attributes).
...
For more operations on raw pointers, see [their API documentation](../std/primitive.pointer.html).
...
A plugin is a dynamic library crate with a designated *registrar* function that registers extensions with `rustc`. Other crates can load these extensions using the crate attribute `#![plugin(...)]`. See the [`rustc::plugin`](../rustc/plugin/index.html) documentation for more about the mechanics of defining and loading a plugin.

If present, arguments passed as `#![plugin(foo(... args ...))]` are not interpreted by rustc itself. They are provided to the plugin through the `Registry`'s [`args` method](../rustc/plugin/registry/struct.Registry.html#method.args).
...
Plugins can extend Rust's syntax in various ways. One kind of syntax extension is the procedural macro. These are invoked the same way as [ordinary macros](macros.html), but the expansion is performed by arbitrary Rust code that manipulates [syntax trees](../syntax/ast/index.html) at compile time.
...
Keep the [`Span`s](../syntax/codemap/struct.Span.html) of everything you parse, for better error reporting. You can wrap [`Spanned`](../syntax/codemap/struct.Spanned.html) around your custom data structures.

Calling [`ExtCtxt::span_fatal`](../syntax/ext/base/struct.ExtCtxt.html#method.span_fatal) will immediately abort compilation. It's better to instead call [`ExtCtxt::span_err`](../syntax/ext/base/struct.ExtCtxt.html#method.span_err) and return [`DummyResult`](../syntax/ext/base/struct.DummyResult.html), so that the compiler can continue and find further errors.

To print syntax fragments for debugging, you can use [`span_note`](../syntax/ext/base/struct.ExtCtxt.html#method.span_note) together with [`syntax::print::pprust::*_to_string`](http://doc.rust-lang.org/syntax/print/pprust/index.html#functions).

The example above produced an integer literal using [`AstBuilder::expr_usize`](../syntax/ext/build/trait.AstBuilder.html#tymethod.expr_usize). As an alternative to the `AstBuilder` trait, `libsyntax` provides a set of [quasiquote macros](../syntax/ext/quote/index.html). They are undocumented and very rough around the edges. However, the implementation may be a good starting point for an improved quasiquote as an ordinary plugin library.

### Lint plugins

Plugins can extend [Rust's lint infrastructure](../reference.html#lint-check-attributes) with additional checks for code style, safety, etc. You can see [`src/test/auxiliary/lint_plugin_test.rs`](https://github.com/rust-lang/rust/blob/master/src/test/auxiliary/lint_plugin_test.rs) for a full example, the core of which is reproduced here:
...
-   one or more `declare_lint!` invocations, which define static [`Lint`](../rustc/lint/struct.Lint.html) structs;

-   a struct holding any state needed by the lint pass (here, none);

-   a [`LintPass`](../rustc/lint/trait.LintPass.html) implementation defining how to check each syntax element. A single `LintPass` may call `span_lint` for several different `Lint`s, but should register them all through the `get_lints` method.
...
Lints defined by plugins are controlled by the usual [attributes and compiler flags](../reference.html#lint-check-attributes), e.g. `#[allow(test_lint)]` or `-A test-lint`. These identifiers are derived from the first argument to `declare_lint!`, with appropriate case and punctuation conversion.
