...
[the #rust-beginners IRC channel on irc.mozilla.org][getting-started.md--irc-beginners] and for
general discussion [the #rust IRC channel on irc.mozilla.org][getting-started.md--irc], which we
can access through [Mibbit][getting-started.md--mibbit]. Then we'll be chatting with other
Rustaceans (a silly nickname we call ourselves) who can help us out. Other great
resources include [the user’s forum][getting-started.md--users] and [Stack Overflow][getting-started.md--stackoverflow].

[getting-started.md--irc-beginners]: irc://irc.mozilla.org/#rust-beginners
[getting-started.md--irc]: irc://irc.mozilla.org/#rust
[getting-started.md--mibbit]: http://chat.mibbit.com/?server=irc.mozilla.org&channel=%23rust-beginners,%23rust
...
[guessing-game.md--cargodoc]: http://doc.crates.io/specifying-dependencies.html
...
left-hand side of a `let` statement is a ‘[pattern][variable-bindings.md--pattern]’, not a
...
When `v` comes into scope, a new [vector][ownership.md--vectors] is created on [the stack][ownership.md--stack],
...
[ownership.md--i32]: #numeric-types
...
```rust
fn main() {
    // Don't worry if you don't understand how `fold` works, the point here is that an
...
You can also create a [reference][mutability.md--ref] to it, using `&x`, but if you want to use the reference to change it, you will need a mutable reference:
...
[mutability.md--ref]: #sec--references-and-borrowing
...
in this case is immutable. Consider, for example, [`Arc<T>`][mutability.md--arc]:
...
[match expressions][structs.md--match].
...
bindings][patterns.md--bindings], [match expressions][patterns.md--match], and other places, too. Let’s go
...
[strings.md--ut]: #sec--unsized-types
...
thread 'main' panicked at 'Some tests failed', /home/steve/src/rust/src/libtest/lib.rs
↳ :247
```
...
[documentation.md--rc-new]: http://doc.rust-lang.org/std/rc/struct.Rc.html#method.new
...
closures are explained in depth [here](#move-closures); basically
...
otherwise (see also: [Choosing Your Guarantees](#sec--choosing-your-guarantees)).
...
Note that [`lock`](http://doc.rust-lang.org/std/sync/struct.Mutex.html#method.lock) method of
...
by using the [corresponding `From` impls](http://doc.rust-lang.org/std/convert/trait.From.html):
...
[error-handling.md--6]: http://doc.rust-lang.org/std/result/index.html
...
[choosing-your-guarantees.md--cell-mod]: http://doc.rust-lang.org/std/cell/index.html
...
extern "C" fn apply(process: Option<extern "C" fn(c_int) -> c_int>, int: c_int) -> c_i
↳ nt {
    match process {
        Some(f) => f(int),
        None    => int * int
...
available via the [`core` crate](http://doc.rust-lang.org/core/index.html). When we’re using the
...
prelude](http://doc.rust-lang.org/core/prelude/v1/index.html). This means that a lot of code will Just
Work:
...
[no-stdlib.md--unwind]: https://github.com/rust-lang/rust/blob/master/src/libpanic_unwind/gcc.rs
...
[syntax-index.md--Iterators]: #sec--iterators
...
[syntax-index.md--Reference (Integer literals)]: http://doc.rust-lang.org/reference.html#integer-literals
