...
If you're on Windows, please download the appropriate [installer][installing-rust.md--install-page].
...
[installing-rust.md--install-page]: https://www.rust-lang.org/install.html
...
[installing-rust.md--users]: https://users.rust-lang.org/
...
language][hello-world.md--expression-oriented language], which means that most things are
expressions, rather than statements. The `;` is used to indicate that this
expression is over, and the next one is ready to begin. Most lines of Rust code
end with a `;`.

[hello-world.md--expression-oriented language]: #expression-oriented-language
...
specified with a [`[lib]` or `[[bin]]`][hello-cargo.md--crates-custom] key in the TOML file.
...
This file is in the [TOML][hello-cargo.md--toml] format. TOML is similar to INI, but has some
...
versions. [Cargo’s documentation][guessing-game.md--cargodoc] contains more details.
...
Inside the thread, all we do is call `eat()` on `p`. Also note that the call to `thread::spawn` lacks a trailing semicolon, making this an expression. This distinction is important, yielding the correct return value. For more details, read [Expressions vs. Statements][dining-philosophers.md--es].

[dining-philosophers.md--es]: functions.html#expressions-vs.-statements
...
We have (2^30^) - 1 in our hypothetical computer with 1GB of RAM. And since
...
145MB uncompressed) or just the [US population data][error-handling.md--13] (2.2MB gzip
...
chapter](http://doc.rust-lang.org/book/hello-cargo.html) and [Cargo's documentation][error-handling.md--14].
...
[`Arc<T>`][choosing-your-guarantees.md--arc] is just a version of `Rc<T>` that uses an atomic reference count (hence, "Arc").
...
similar to an `&mut [T]`[^choosing-your-guarantees.md--3], but, again, the borrow checking is at runtime.
...
because the assigned value can have [just one owner](#sec--ownership), and any
...
The expression is an [iterator][loops.md--iterator]. The iterator gives back a series of
